...
const SidebarLink = ({ active, icon, label, onClick }) => (
  <button 
    onClick={onClick}
    className={`w-full flex items-center gap-3 px-3.5 py-3 rounded-xl transition-all text-sm font-medium ${
      active 
      ? 'bg-violet-50 dark:bg-violet-500/10 text-violet-600 dark:text-violet-400' 
      : 'text-stone-500 dark:text-stone-400 hover:bg-stone-100 dark:hover:bg-stone-800/50 hover:text-stone-700 dark:hover:text-stone-300'
    }`}
  >
...
const CustomerLayout = ({ children, sidebarContent, onSearch, searchQuery, setSearchQuery }) => {
  ...
  return (
    <div className="min-h-screen bg-stone-50 dark:bg-stone-950 flex font-sans">
      {/* Desktop Sidebar */}
      <aside className="w-64 bg-white dark:bg-stone-900 border-r border-stone-200 dark:border-stone-800 flex flex-col hidden lg:flex fixed h-full z-20">
        <div className="px-6 pt-7 pb-5">
          <button 
            className="flex items-center gap-2.5 group" 
            onClick={() => navigate('/home')}
          >
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-violet-600 to-pink-600 flex items-center justify-center text-white shadow-lg shadow-violet-500/30">
        ...
        <nav className="px-3.5 space-y-1 mb-6">
        ...
        {sidebarContent && (
            <div className="flex-1 overflow-y-auto px-3.5 custom-scrollbar">
                {sidebarContent}
            </div>
        )}
        ...
        <div className="p-4 mt-auto">
          <div className="p-4 rounded-2xl bg-stone-50 dark:bg-stone-800/50 border border-stone-100 dark:border-stone-700">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-9 h-9 rounded-full bg-violet-100 dark:bg-violet-500/20 flex items-center justify-center text-violet-600 text-sm font-semibold">
                {user?.name?.[0] || 'U'}
              </div>
      ...
      {/* Sidebar - Mobile */}
      <aside className={`fixed inset-y-0 left-0 w-64 bg-white dark:bg-stone-900 z-50 transform transition-transform duration-200 lg:hidden ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <main className="flex-1 lg:ml-64 flex flex-col h-screen overflow-hidden">
        {/* Header - Glassmorphic */}
        <header className="glass border-b border-stone-200 dark:border-stone-800 px-4 lg:px-6 py-3.5 flex items-center justify-between z-10 sticky top-0">
          ...
          <div className="flex-1 max-w-lg relative mx-3">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 rounded-xl bg-stone-100/70 dark:bg-stone-800/70 border border-transparent focus:border-violet-500/40 focus:ring-2 focus:ring-violet-500/20 outline-none transition-all text-sm text-stone-700 dark:text-stone-200 placeholder-stone-400"
            />
          </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="relative cursor-pointer group p-2 hover:bg-stone-100 dark:hover:bg-stone-800 rounded-lg transition-colors" onClick={() => navigate('/cart')}>
                <ShoppingCart className="h-[18px] w-[18px] text-stone-500 group-hover:text-violet-600 transition-colors" />
                {items.length > 0 && (
                  <span className="absolute top-1 right-1 min-w-[16px] h-4 bg-violet-600 text-white text-[10px] font-semibold flex items-center justify-center rounded-full px-1 border-2 border-white dark:border-stone-900">
                      {items.length}
                  </span>
                )}
            </div>
        ...
        {/* Dynamic Content */}
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-stone-50/50 dark:bg-stone-950/50">
            {children}
        </div>
      </main>
    </div>
